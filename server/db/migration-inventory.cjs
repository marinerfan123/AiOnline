...
/**
 * Detect duplicate version numbers already present in the existing migration
 * chain (e.g. two files numbered 0016). G0-04 requires every migration to have a
 * UNIQUE reserved sequence; a duplicate in the chain is a governance violation
 * that must be surfaced (not silently "sorted past" by filename order).
 * @param {Array<{version:string,name:string,filename:string}>} migrations
 * @returns {Array<{version:string,files:string[]}>}
 */
function getDuplicateVersions(migrations) {
  const byVersion = new Map();
  for (const m of migrations) {
    if (!byVersion.has(m.version)) byVersion.set(m.version, []);
    byVersion.get(m.version).push(m.filename);
  }
...
function buildInventory() {
  ...
  return {
    ...
    duplicates: getDuplicateVersions(migrations),
    migrations,
  };
}
...
module.exports = { discoverMigrations, getHeadVersion, getNextVersion, getDuplicateVersions, buildInventory };
