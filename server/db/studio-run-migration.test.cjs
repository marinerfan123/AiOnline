'use strict';
/**
 * M05-D1 — Migration 0016 (studio run engine) schema tests.
...
test('migration 0016 creates the durable studio run engine schema', { concurrency: 1 }, async () => {
...
test('migration 0016: FKs, idempotency uniqueness, and ON DELETE semantics', { concurrency: 1 }, async () => {
  const { dbName, pg } = await bootstrap();
  try {
