...
test('runUploadTick加载完整batch上下文并按concurrency限制并发',async()=>{let a=0,max=0,loaded=0;const items=Array.from({length:6},(_,i)=>({item_id:`i${i}`,batch_id:'b',item_index:i,lease_version:1,provider_url:'x'}));const deps={claimGeneratedItems:async()=>items,loadItemContext:async(_pg,id)=>{loaded++;return{item_id:id,batch_id:'b',item_index:Number(id.slice(1)),user_id:'u',provider_url:'x',request_payload:{}}},transitionItem:async(_p,x)=>({status:x.to}),uploadToOss:async()=>{a++;max=Math.max(max,a);await new Promise(r=>setTimeout(r,5));a--;return{ossUrl:'u'}},settleHold:async()=>({}),reconcileBatch:async()=>({})};const r=await runUploadTick({}, {workerId:'u',concurrency:2},deps);assert.equal(r.claimed,6);assert.equal(loaded,6);assert.ok(max<=2)});
