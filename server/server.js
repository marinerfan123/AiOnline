...
import startMediaWorkersMod from './modules/media/startMediaWorkers.cjs';
import runtimeSettingsMod from './runtimeSettings.cjs';
...
// local-disk 媒体读取面：GET /local-media/<objectKey>（key 每段 encode；生产走 OSS，本面仅测试/离线链）。
const localMediaRead = ossMod.createLocalMediaStore({});
...
server.listen(PORT, '0.0.0.0', async () => {
  ...
  // placement 由 settings.app.mediaNormalizationPlacement 控制：'api' 在 API 进程内跑，
  // 'worker' 交由 media-worker-entry.cjs 独立进程跑，'off' 关闭——可随时切换回退。
  if (String(process.env.MEDIA_JOBS_WORKER || '') === '1') {
    try {
      const rt = await runtimeSettingsMod.getRuntimeSettings(pgPool);
      if (rt.mediaNormalizationPlacement !== 'api') {
        console.log(`[media-worker] placement=${rt.mediaNormalizationPlacement}，API 进程不启动归一化（交由独立 media-worker 进程或已关闭）`);
      } else {
        startMediaWorkersMod.startMediaWorkers({
          pgPool,
          nodeId: NODE_ID,
          ossMod,
          mediaExecMod,
