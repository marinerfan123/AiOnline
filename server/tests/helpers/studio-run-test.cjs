...
async function createDb() {
  ...
  // The name MUST contain "test": test-app.cjs spawnTestServer() refuses to
  // fork the real server against a DB that fails /test/i (fail-closed safety
  // gate). A name like "moling_m05d_*" trips that gate even though the DB is
  // a throwaway per-test database, breaking the API integration suite.
...
/** Fresh DB + all migrations through 0016. Returns { dbName, pg }. */
