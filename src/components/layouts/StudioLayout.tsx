...
export function StudioLayout() {
  ...
  const canvasMatch = useMatch('/studio/:projectId');
  const immersiveCanvas = Boolean(canvasMatch);
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-ml2-surface-0 text-ml2-text">
      ...
      <div className="flex flex-1 flex-col min-w-0">
        {!immersiveCanvas && <MobileDockBar title="创作工作室" onOpen={() => setMobileDockOpen(true)} />}
