    ...
    >
      ...
      <div className="p-1.5">
        ...
        {def.id === 'image' && (
          data.assetId
            ? <AssetPreview assetId={data.assetId} kind="image" />
            : <AssetPlaceholder label="未选择图片素材" hint="在 Inspector 中绑定图片资产（当前版本）" />
        )}
        {def.id === 'audio' && (
          <AssetPlaceholder label={data.assetId ? '音频素材' : '未选择音频素材'} hint={data.assetId ? String(params.voiceId || '波形（G06）') : '在 Inspector 中绑定音频资产'} />
        )}
        {def.id === 'video-clip' && (
          data.assetId
            ? <AssetPreview assetId={data.assetId} kind="video" />
        ...
        {def.id === 'storyboard' && (
          data.assetId
            ? <AssetPreview assetId={data.assetId} kind="image" />
            : <AssetPlaceholder label={params.shotId ? `镜头 ${String(params.shotId)} 候选待生成` : '分镜候选板'} hint="绑定 Shot 或先生成候选图 (G13)" />
        )}
        {def.id === 'output' && (
        ...
        {isFrame && (
          <div className="grid h-16 place-items-center rounded-md border border-dashed border-ml2-border bg-ml2-surface-0">
            <span className="text-[10px] text-ml2-text-3">{data.frameLabel || '框架 / 分组'}</span>
          </div>
        )}
      </div>
